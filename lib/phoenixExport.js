 ...
 */
function cleanupGeometry(node, pathsToHide, maxLevel = 999, currLevel = 0) {
...
function deduplicate(gltf) {
  ...
  console.log(
    `      Initial number of meshes/accessors: ${gltf.meshes.length}/${
  ...
  gltf.meshes = kept;
  console.log(
    `      New number of meshes/accessors: ${gltf.meshes.length}/${
      gltf.accessors.length
    }`,
  );
...
function set_visible_recursively(node) {
  ...
  if (node.fVolume.fNodes) {
    for (let j = 0; j < node.fVolume.fNodes.arr.length; j++) {
      const snode = node.fVolume.fNodes.arr[j];
...
function keep_only_subpart(volume, paths) {
  ...
  let anyfound = false;
  for (let j = 0; j < volume.fNodes.arr.length; j++) {
    const snode = volume.fNodes.arr[j];
    if (matches(snode.fName, paths)) {
      ...
      const subpartfound = keep_only_subpart(snode.fVolume, paths);
      if (subpartfound) {
